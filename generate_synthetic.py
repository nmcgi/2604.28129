    ...
    if args.extended:
        ...
        for i, (cat, desc) in enumerate(attack_cats):
            for _ in range(base + (1 if i < rem else 0)):
                ...
                conv   = generate(ATTACK_SYSTEM + EXTENDED_PIVOT_EXTRA, prompt, client, args.gen_model)
                conv["category"] = cat
                conv["split"]    = "extended"
                ...
                idx += 1
                if idx % 50 == 0:
                    print(f"[extended] {idx} conversations written")
        ...
        for split, cat, is_attack, domain, turns in jobs:
            i = counters[split]
            if is_attack:
                prompt = make_attack_prompt(cat, ATTACK_CATEGORIES[cat], domain, turns)
                conv = generate(ATTACK_SYSTEM, prompt, client, args.gen_model)
            else:
                prompt = make_benign_prompt(cat, domain, turns)
                conv = generate(BENIGN_SYSTEM, prompt, client, args.gen_model)
            conv["category"] = cat
            conv["split"] = split
            ...
            counters[split] += 1
            if i % 50 == 0:
                print(f"[{split}] {i} conversations written")
